...
#include "syscalls.h"  /* SeraphSleep, SeraphCreateThread — no Win32 Sleep/CreateThread */
...
static ID2D1HwndRenderTarget*    g_espRT        = NULL;
...
static ID2D1SolidColorBrush*     g_espBrushEnemy   = NULL;
static ID2D1SolidColorBrush*     g_espBrushAlly    = NULL;
static ID2D1SolidColorBrush*     g_espBrushUnknown = NULL;
static ID2D1SolidColorBrush*     g_espBrushSkeleton = NULL;
static ID2D1SolidColorBrush*     g_espBrushFov      = NULL;
static ID2D1BitmapBrush*         g_espBrushBgChecker = NULL;
static ID2D1SolidColorBrush*     g_espBrushOutline  = NULL;
...
static void EspOverlay_RenderFrame(void)
{
    ...
    /* Schindler scan and matchmaking scan run in the background player thread.
    ...
    if (!hasMatrix) hasMatrix = ESP_ReadMatrices(view, proj);
    ...
    if (ns > 0 && hasMatrix && (drawSkel || drawBoxes || drawHealth || drawShield || drawDistance || drawName) && g_espBrushSkeleton) {
        ...
        typedef struct { int a; int b; } SkelEdge;
        /* Verified bone mapping for current Destiny 2 build:
         *   18 = head         13 = neck          14 = collarbone/chest center
         *   11 = upper-spine   8 = mid-spine      5 = lower-spine   1 = pelvis
         *   12 = left-clavicle 15 = left-shoulder 16 = left-elbow   19 = left-hand
         *   17 = right-shldr  20 = right-elbow   21 = right-hand    22 = weapon
         *   3  = left-hip      6 = left-knee      9 = left-ankle/foot
         *   4  = right-hip     7 = right-knee    10 = right-ankle/foot
         */
        static const SkelEdge SKELETON_EDGES[] = {
            /* Spine chain */
            {18, 13}, {13, 11}, {11, 8}, {8, 5}, {5, 1},

            /* Left Arm (screen right) — connect shoulder directly to neck (13) */
            {13, 15}, {15, 16}, {16, 19},

            /* Right Arm (screen left) — connect shoulder directly to neck (13) */
            {13, 17}, {17, 20}, {20, 21},
        ...
        static const int SKELETON_EDGE_COUNT = sizeof(SKELETON_EDGES) / sizeof(SKELETON_EDGES[0]);
        ...
        for (int i = 0; i < ns; i++) {
            ...
            if (drawSkel) {
                ...
                if (projectedBones > 0) {
                    for (int k = 0; k < SKELETON_EDGE_COUNT; k++) {
                        int a = SKELETON_EDGES[k].a;
                        int b = SKELETON_EDGES[k].b;
                        if ((UINT32)a < s.boneCount && (UINT32)b < s.boneCount) {
                            if (projOk[a] && projOk[b]) {
                                g_espRT->DrawLine({projX[a], projY[a]}, {projX[b], projY[b]}, g_espBrushSkeleton, 2.2f);
                            }
                        }
                    }
                }
                ...
                if (topOk && botOk) {
                    ...
                    if (boxH > 5.0f) {
                        ...
                        if (s.health >= 0.0f && (drawHealth || drawShield)) {
                            ...
                            /* If health is active but shield is NOT, make health bar represent the combined health + shield value.
                             * Typically, Destiny 2 player health is 70HP (35% of total) and shield is 130HP (65% of total).
                             * A realistic combined representation would be: total = (health * 70 + shield * 130) / 200.0f. */
                            float combinedRaw = hpRaw;
                            if (drawHealth && !drawShield) {
                                combinedRaw = (hpRaw * 70.0f + shRaw * 130.0f) / 200.0f;
                                if (combinedRaw > 1.0f) combinedRaw = 1.0f;
                                if (combinedRaw < 0.0f) combinedRaw = 0.0f;
                            }
                            ...
                            if (drawHealth) {
                                ...
                                /* Colour calculation based on the actual drawn value (combined or raw) */
                                float colorVal = (drawHealth && !drawShield) ? combinedRaw : hpRaw;

                                /* Colour: pure vibrant Green (full) → Yellow (half) → Red (empty) */
                                float r_col, g_col, b_col = 0.0f;
                                if (colorVal >= 0.5f) {
                                    float k = (colorVal - 0.5f) * 2.0f;
                                    r_col = 1.0f - k;
                                    g_col = 1.0f;
                                } else {
                                    float k = colorVal * 2.0f;
                                    r_col = 1.0f;
                                    g_col = k;
                                }
                                ...
                                if (g_espBrushHpDynamic) {
                                    ...
                                    float fillH   = (b2 - t) * (drawHealth && !drawShield ? combinedRaw : hpRaw);
        ...
        if ((nowMs - s_lastFgCheck) >= 33u) {
            ...
            if (!hGame || !IsWindow(hGame)) {
                ...
                if (nowMs - lastFindMs >= 1000) {
                    lastFindMs = nowMs;
                    WCHAR szD2Title[16];
                    szD2Title[0] = L'D'; szD2Title[1] = L'e'; szD2Title[2] = L's';
                    szD2Title[3] = L't'; szD2Title[4] = L'i'; szD2Title[5] = L'n';
                    szD2Title[6] = L'y'; szD2Title[7] = L' '; szD2Title[8] = L'2';
                    szD2Title[9] = L'\0';
                    hGame = FindWindowW(NULL, szD2Title);
                }
            }
            BOOL isFocused = (hGame && (hForeground == hGame)) || (fgPid == GetCurrentProcessId());
            static DWORD s_unfocusedStart = 0;
            if (isFocused) {
                s_unfocusedStart = 0;
                shouldShow = TRUE;
            } else {
                if (s_unfocusedStart == 0) s_unfocusedStart = nowMs;
                if (nowMs - s_unfocusedStart >= 150) {
                    shouldShow = FALSE;
                } else {
                    shouldShow = TRUE;
                }
    ...
    while (TRUE) {
        if (g_espTargetState && !g_espCurrentState) {
            if (InterlockedCompareExchange(&g_espRunning, 1, 0) == 0) {
                ...
                g_espThread = SeraphCreateThread(EspOverlay_Thread, NULL);
...
extern "C" void EspOverlay_Start(void)
{
    ...
    if (!g_espManagerThread) g_espManagerThread = SeraphCreateThread(EspManagerThread, NULL);
}
