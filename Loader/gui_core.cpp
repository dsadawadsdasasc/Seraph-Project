...
/* Per-ability debug log helper — writes directly to abilities_debug.log if ENABLE_DEBUG is active */
#include <stdio.h>
#include <stdarg.h>
static void LogAbilityMsg(const char* fmt, ...) {
#ifdef ENABLE_DEBUG
    char buf[512];
    va_list args;
    va_start(args, fmt);
    ...
    SYSTEMTIME st;
    GetLocalTime(&st);
    snprintf(line, sizeof(line), "[%02d:%02d:%02d.%03d][ABILITY] %s\n",
             st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, buf);
             
    WriteLogFileEx("abilities_debug.log", line);
#endif
}
#include "attach.h"
#include "patch.h"
...
static bool  g_classAbilityActive=false;
static int   g_classAbilityMult=1;   /* slider 1..20 -> 1x..20x */
static bool  g_meleeAbilityActive=false;
static int   g_meleeAbilityMult=1;   /* slider 1..20 -> 1x..20x */
static bool  g_grenadeAbilityActive=false;
static int   g_grenadeAbilityMult=1; /* slider 1..20 -> 1x..20x */
static bool  g_superAbilityActive=false;
static int   g_superAbilityMult=1;   /* slider 1..20 -> 1x..20x */
/* static bool  g_chamsActive=false;  // CHAMS: temporarily disabled */
static bool  g_espActive=false;    /* ESP overlay (entity boxes) toggle */
static bool  g_espTeamCheck=false;  /* FALSE = show all boxes by default */
static bool  g_aimbotActive=false; /* Aimbot toggle */
static int   g_aimbotSmooth=50;    /* Aimbot smoothing slider 1-100 (default=50≈smooth 8.0) */
static bool  g_aimbotShowFov=false; /* Show FOV circle toggle */
static int   g_aimbotFovSize=50;    /* FOV size slider 1-100 (default=50≈328px) */
static bool  g_aimbotHeadTarget=true; /* TRUE=aim head, FALSE=aim body */
...
static bool  g_aimbotTeamCheck=false;         /* FALSE = aimbot targets all entities by default */
static bool  g_spinbotActive=false;           /* Spinbot toggle */
static bool  g_aimbotTriggerbot=false;        /* Triggerbot toggle */
#ifdef SERAPH_DMA_BUILD
...
static bool  g_wstatReloadActive=false;
static int   g_wstatReloadMult=10;     /* 1..10 */
static bool  g_wstatHandlingActive=false;
static int   g_wstatHandlingMult=2;    /* 1..10 */
static bool  g_wstatRangeActive=false;
static int   g_wstatRangeMult=2;       /* 1..10 */
static bool  g_wstatAmmoActive=false;
static int   g_wstatAmmoValue=99;      /* 1..99 */
...
static bool  g_thirdPersonActive=false; /* Third Person toggle */
static int   g_thirdPersonDistance=47;  /* Third Person distance slider 1-100 (default 47 ≈ 15.0f) */
...
static bool  g_playerMenuExpanded = false;
static float g_playerMenuOpacity = 0.0f;
...
static void DeleteConfigJson(const std::string& name) {
    std::wstring dir = GetConfigDir();
    if (dir.empty()) return;
...
static void ApplyLoadedSettings() {
    ...
    ThirdPerson_SetDistance(2.0f + 28.0f * ((float)(g_thirdPersonDistance - 1) / 99.0f));
    ThirdPerson_SetEnabled(g_thirdPersonActive ? TRUE : FALSE);
    ...
    /* Direct-write Weapon Stats */
    WeaponStats_SetReloadMult(g_wstatReloadActive ? (float)g_wstatReloadMult : 0.0f);
    WeaponStats_SetHandlingMult(g_wstatHandlingActive ? (float)g_wstatHandlingMult : 0.0f);
    WeaponStats_SetRangeMult(g_wstatRangeActive ? (float)g_wstatRangeMult : 0.0f);

    /* Encrypted Ammo custom value setting */
    Ammo_SetEnabled(g_wstatAmmoActive ? TRUE : FALSE);
    Ammo_SetValue(g_wstatAmmoActive ? (UINT32)g_wstatAmmoValue : 0);
    WeaponStats_SetAmmoOverride(g_wstatAmmoActive ? TRUE : FALSE, (UINT32)g_wstatAmmoValue);
...
static void SaveConfigJson(const std::string& name) {
    ...
    j["settings"]["spinbotActive"] = g_spinbotActive;
    ...
    j["settings"]["aimbotTriggerbot"] = g_aimbotTriggerbot;
#ifdef SERAPH_DMA_BUILD
...
#endif
    j["settings"]["wstatReloadActive"] = g_wstatReloadActive;
    j["settings"]["wstatReloadMult"] = g_wstatReloadMult;
    j["settings"]["wstatHandlingActive"] = g_wstatHandlingActive;
    j["settings"]["wstatHandlingMult"] = g_wstatHandlingMult;
    j["settings"]["wstatRangeActive"] = g_wstatRangeActive;
    j["settings"]["wstatRangeMult"] = g_wstatRangeMult;
    j["settings"]["wstatAmmoActive"] = g_wstatAmmoActive;
    j["settings"]["wstatAmmoValue"] = g_wstatAmmoValue;
    ...
    j["settings"]["classAbilityActive"] = g_classAbilityActive;
    j["settings"]["classAbilityMult"] = g_classAbilityMult;
    j["settings"]["meleeAbilityActive"] = g_meleeAbilityActive;
    j["settings"]["meleeAbilityMult"] = g_meleeAbilityMult;
    j["settings"]["grenadeAbilityActive"] = g_grenadeAbilityActive;
    j["settings"]["grenadeAbilityMult"] = g_grenadeAbilityMult;
    j["settings"]["superAbilityActive"] = g_superAbilityActive;
    j["settings"]["superAbilityMult"] = g_superAbilityMult;
    j["settings"]["thirdPersonActive"] = g_thirdPersonActive;
    j["settings"]["thirdPersonDistance"] = g_thirdPersonDistance;
...
static void LoadConfigJson(const std::string& name, bool quiet = false) {
    ...
    try {
        ...
        if (j.contains("settings")) {
            ...
            if (s.contains("spinbotActive")) g_spinbotActive = s["spinbotActive"];
            ...
            if (s.contains("aimbotTriggerbot")) g_aimbotTriggerbot = s["aimbotTriggerbot"];
#ifdef SERAPH_DMA_BUILD
...
#endif
            if (s.contains("wstatReloadActive")) g_wstatReloadActive = s["wstatReloadActive"];
            if (s.contains("wstatReloadMult")) g_wstatReloadMult = s["wstatReloadMult"];
            if (s.contains("wstatHandlingActive")) g_wstatHandlingActive = s["wstatHandlingActive"];
            if (s.contains("wstatHandlingMult")) g_wstatHandlingMult = s["wstatHandlingMult"];
            if (s.contains("wstatRangeActive")) g_wstatRangeActive = s["wstatRangeActive"];
            if (s.contains("wstatRangeMult")) g_wstatRangeMult = s["wstatRangeMult"];
            if (s.contains("wstatAmmoActive")) g_wstatAmmoActive = s["wstatAmmoActive"];
            if (s.contains("wstatAmmoValue")) g_wstatAmmoValue = s["wstatAmmoValue"];
            ...
            if (s.contains("classAbilityActive")) g_classAbilityActive = s["classAbilityActive"];
            if (s.contains("classAbilityMult")) g_classAbilityMult = s["classAbilityMult"];
            if (s.contains("meleeAbilityActive")) g_meleeAbilityActive = s["meleeAbilityActive"];
            if (s.contains("meleeAbilityMult")) g_meleeAbilityMult = s["meleeAbilityMult"];
            if (s.contains("grenadeAbilityActive")) g_grenadeAbilityActive = s["grenadeAbilityActive"];
            if (s.contains("grenadeAbilityMult")) g_grenadeAbilityMult = s["grenadeAbilityMult"];
            if (s.contains("superAbilityActive")) g_superAbilityActive = s["superAbilityActive"];
            if (s.contains("superAbilityMult")) g_superAbilityMult = s["superAbilityMult"];
            if (s.contains("thirdPersonActive")) g_thirdPersonActive = s["thirdPersonActive"];
            if (s.contains("thirdPersonDistance")) g_thirdPersonDistance = s["thirdPersonDistance"];
...
static DWORD WINAPI FeatureInitThread_Impl(LPVOID) {
    ...
    if (cr3 && d2Base) {
        /* With TimeDateStamp validation in place, ASLR reuse is no longer a risk.
         * The cache file is safely validated and kept across restarts of the same game build. */
        AobCacheRec rec;
        BOOL cacheHit = AobCache_Read(d2Base, d2Tds, &rec);

        BYOVD_SCAN_ENTRY feAll[] = {
            { k_dmg_pat,    k_dmg_mask,    (UINT8)13,  0x20000000ULL, 0 },
            { k_hrg_pat,    k_hrg_mask,    (UINT8)13,  0x20000000ULL, 0 },
            { k_ik_pat,     k_ik_mask,     (UINT8) 9,  0x20000000ULL, 0 },
            { k_cam_pat,    k_cam_mask,    (UINT8) 8,  0x20000000ULL, 0 },
            { k_hkp_pat,    k_hkp_mask,    (UINT8) 9,  0x20000000ULL, 0 },
            { k_rof_pat,    k_rof_mask,    (UINT8)11,  0x20000000ULL, 0 },
            { k_ia_pat,     k_ia_mask,     (UINT8)11,  0x20000000ULL, 0 },
            { k_ntb_pat,    k_ntb_mask,    (UINT8) 8,  0x20000000ULL, 0 },
            { k_nja_pat,    k_nja_mask,    (UINT8) 6,  0x20000000ULL, 0 },
#ifdef SERAPH_EXCLUDE_KILLAURA
            { NULL,         NULL,          0,          0,             0 },
#else
            { k_killaura_pat, k_killaura_mask, (UINT8)KILLAURA_AOB_LEN, 0x20000000ULL, 0 },
#endif
            { k_gs_pat,    k_gs_mask,    (UINT8)  8,  0x20000000ULL, 0 },
            { k_gsize_pat, k_gsize_mask, (UINT8)  8,  0x20000000ULL, 0 },
            { k_ib_pat,    k_ib_mask,    (UINT8)  6,  0x20000000ULL, 0 },
            { k_itar_pat,  k_itar_mask,  (UINT8) 12,  0x20000000ULL, 0 },
            { k_ammo_pat,  k_ammo_mask,  (UINT8) 11,  0x20000000ULL, 0 },
            { k_cloner_pat, k_cloner_mask,(UINT8) CLONER_AOB_LEN, 0x20000000ULL, 0 },
            { k_brick_pat, k_brick_mask, (UINT8)BRICK_AOB_LEN, 0x20000000ULL, 0 },
            { k_hs_pat,    k_hs_mask,    (UINT8)HS_AOB_LEN,    0x20000000ULL, 0 },
            { k_ms_pat,    k_ms_mask,    (UINT8)MS_AOB_LEN,    0x20000000ULL, 0 },
            { k_norecoil_pat, k_norecoil_mask, (UINT8)13,      0x20000000ULL, 0 },
            { k_silent_pat, k_silent_mask, (UINT8)23,          0x20000000ULL, 0 },
            { k_thirdperson_pat, k_thirdperson_mask, (UINT8)7, 0x20000000ULL, 0 },
            { NULL, NULL, 0, 0, 0 }, /* spinbot dummy entries */
        };
        const int N_ALL = (int)(sizeof(feAll)/sizeof(feAll[0]));

        /* Hardcoded offset initialization (Securely Decrypted via SecureReadStatic) */
        feAll[0].result  = d2Base + SecureReadStatic(&OBF_OFF_Damage);
        feAll[1].result  = d2Base + SecureReadStatic(&OBF_OFF_HealthRegen);
        feAll[2].result  = d2Base + SecureReadStatic(&OBF_OFF_InstaKill);
        feAll[3].result  = d2Base + SecureReadStatic(&OBF_OFF_Fly_Cam);
        feAll[4].result  = d2Base + SecureReadStatic(&OBF_OFF_Havok);
        feAll[5].result  = d2Base + SecureReadStatic(&OBF_OFF_RapidFire);
        feAll[6].result  = d2Base + SecureReadStatic(&OBF_OFF_InstAbils);
        feAll[7].result  = d2Base + SecureReadStatic(&OBF_OFF_NoTurnBack);
        feAll[8].result  = d2Base + SecureReadStatic(&OBF_OFF_NoJoinAllies);
        feAll[9].result  = 0; /* KillAura (Not Found / Disabled) */
        feAll[10].result = d2Base + SecureReadStatic(&OBF_OFF_GameSpeed);
        feAll[11].result = d2Base + SecureReadStatic(&OBF_OFF_Guardian);
        feAll[12].result = d2Base + SecureReadStatic(&OBF_OFF_ImmuneBoss);
        feAll[13].result = d2Base + SecureReadStatic(&OBF_OFF_InteractAura);
        feAll[14].result = d2Base + SecureReadStatic(&OBF_OFF_InfiniteAmmo);
        feAll[15].result = d2Base + SecureReadStatic(&OBF_OFF_PlayerCloner);
        feAll[16].result = d2Base + SecureReadStatic(&OBF_OFF_AmmoBrick);
        feAll[17].result = d2Base + SecureReadStatic(&OBF_OFF_HSpeed);
        feAll[18].result = d2Base + SecureReadStatic(&OBF_OFF_MSpeed);
        feAll[19].result = d2Base + SecureReadStatic(&OBF_OFF_NoRecoil);
        feAll[20].result = d2Base + SecureReadStatic(&OBF_OFF_SilentAim);
        feAll[21].result = d2Base + SecureReadStatic(&OBF_OFF_ThirdPerson);
        feAll[22].result = d2Base + SecureReadStatic(&OBF_OFF_SpinBot);

        /* Commented out AOB scans and cache checks
#ifdef SERAPH_DMA_BUILD
        BOOL allFromCache = FALSE;
#else
        BOOL allFromCache = cacheHit && rec.features_valid;
#endif
        if (allFromCache) {
            feAll[0].result  = rec.dmg_va;
            feAll[1].result  = rec.hrg_va;
            feAll[2].result  = rec.ik_va;
            feAll[3].result  = rec.cam_aob_va;
            feAll[4].result  = rec.hkp_aob_va;
            feAll[5].result  = rec.rof_va;
            feAll[6].result  = rec.ia_va;
            feAll[7].result  = rec.ntb_va;
            feAll[8].result  = rec.nja_va;
            feAll[9].result  = rec.killaura_va;
            feAll[10].result = rec.gs_va;
            feAll[11].result = rec.gsize_va;
            feAll[12].result = rec.ib_va;
            feAll[13].result = rec.itar_va;
            feAll[14].result = rec.ammo_va;
            feAll[15].result = rec.cloner_va;
            feAll[16].result = rec.brick_va;
            feAll[17].result = rec.hs_va;
            feAll[18].result = rec.movespeed_va;
            feAll[19].result = rec.norecoil_va;
            feAll[20].result = rec.silent_va;
        } else {
            BYOVD_SCAN_ENTRY feText[22] = {
                feAll[0],  feAll[1],  feAll[2],  feAll[3],  feAll[4],
                feAll[5],  feAll[6],  feAll[7],  feAll[8],  feAll[9],
                feAll[10], feAll[11], 
                feAll[12], feAll[13], 
                feAll[14], feAll[15], feAll[16], feAll[17], feAll[18],
                feAll[19], feAll[20], feAll[21]
            };
            const int N_TEXT = (int)(sizeof(feText)/sizeof(feText[0]));

            BYOVD_SCAN_ENTRY feData[2] = {
                feAll[10], feAll[11]
            };
            const int N_DATA = (int)(sizeof(feData)/sizeof(feData[0]));

            BYOVD_LOCK();
            {
                char hdr[160];
                wsprintfA(hdr, "FIT: ScanMultiPatternText START N=%d", N_TEXT);
                WriteLogFile(hdr);
                BYOVD_ScanMultiPatternText(cr3, d2Base, feText, N_TEXT);
                WriteLogFile("FIT: ScanMultiPatternText DONE");

                wsprintfA(hdr, "FIT: ScanMultiPatternData START N=%d", N_DATA);
                WriteLogFile(hdr);
                BYOVD_ScanMultiPatternData(cr3, d2Base, feData, N_DATA);
                WriteLogFile("FIT: ScanMultiPatternData DONE");
            }
            BYOVD_UNLOCK();

            for (int _i = 0; _i < 12; _i++) feAll[_i].result = feText[_i].result;
            if (!feAll[10].result) feAll[10].result = feData[0].result;
            if (!feAll[11].result) feAll[11].result = feData[1].result;
            feAll[12].result = feText[12].result;
            feAll[13].result = feText[13].result;
            feAll[14].result = feText[14].result;
            feAll[15].result = feText[15].result;
            feAll[16].result = feText[16].result;
            feAll[17].result = feText[17].result;
            feAll[18].result = feText[18].result;
            feAll[19].result = feText[19].result;
            feAll[20].result = feText[20].result;
            feAll[21].result = feText[21].result;
        }
        */
#ifndef SERAPH_DMA_BUILD
        {
            static const char *feNames[] = {
                "Damage", "HealthRegen", "InstaKill", "Fly_Cam",
                "Havok", "RapidFire", "InstAbils", "NoTurnBack", "NoJoinAllies",
                "KillAura", "GameSpeed", "Guardian", "ImmuneBoss",
                "InteractAura", "InfiniteAmmo", "PlayerCloner",
                "AmmoBrick", "HSpeed", "MSpeed", "NoRecoil", "SilentAim"
            };
            int nNames = (int)(sizeof(feNames)/sizeof(feNames[0]));
            for (int _fi = 0; _fi < N_ALL && _fi < nNames; _fi++)
                BYOVD_LogScanResult(feNames[_fi], feAll[_fi].result);
        }
#endif
        Damage_SetPreScanResult           (feAll[0].result);
        HealthRegen_SetPreScanResult      (feAll[1].result);
        InstaKill_SetPreScanResult        (feAll[2].result);
        Fly_SetCamPreScanResult           (feAll[3].result);
        Havok_SetHkpPreScanResult         (feAll[4].result);
        RapidFire_SetPreScanResult        (feAll[5].result);
        InstantAbilities_SetPreScanResult (feAll[6].result);
        NoTurnBack_SetPreScanResult       (feAll[7].result);
        NoJoinAllies_SetPreScanResult     (feAll[8].result);
        KillAura_SetPreScanResult         (feAll[9].result);
        GameSpeed_SetPreScanResult        (feAll[10].result);
        Guardian_SetPreScanResult         (feAll[11].result);
        ImmuneBoss_SetPreScanResult       (feAll[12].result);
        InteractAura_SetPreScanResult     (feAll[13].result);
        InfiniteAmmo_SetPreScanResult     (feAll[14].result);
        PlayerCloner_SetPreScanResult     (feAll[15].result);
        AmmoBrick_SetPreScanResult        (feAll[16].result);
        HSpeed_SetPreScanResult           (feAll[17].result);
        MSpeed_SetPreScanResult           (feAll[18].result);
        NoRecoil_SetPreScanResult         (feAll[19].result);
        SilentAim_SetPreScanResult        (feAll[20].result);
        ThirdPerson_SetPreScanResult      (feAll[21].result);
        SpinBot_SetPreScanResult          (feAll[22].result);
        {
            struct { int idx; const char* name; } req[] = {
                { 0,"Damage"},      { 1,"HealthRegen"}, { 2,"InstaKill"},
                { 3,"Fly cam"},     { 4,"Havok hkp"},   { 5,"RapidFire"},
                { 6,"InstAbils"},   { 7,"NoTurnBack"},  { 8,"NoJoinAllies"},
                { 9,"KillAura"},    {10,"GameSpeed"},   {11,"Guardian"},
                {12,"ImmuneBoss"},  {13,"InteractAura"},
                {14,"InfiniteAmmo"},{15,"PlayerCloner"}, {16,"AmmoBrick"},
                {17,"HandlingSpeed"}, {18,"MovementSpeed"}, {19,"NoRecoil"}, {20,"SilentAim"}
            };
            for (int i = 0; i < (int)(sizeof(req)/sizeof(req[0])); i++) {
                if (!feAll[req[i].idx].result) {
                    char b[160];
                    wsprintfA(b,"AOB MISSING multi-scan: '%s' (idx=%d) - will retry in OnAttach",
                             req[i].name, req[i].idx);
                    WriteLogFile(b);
                }
            }
        }
        /* Commented out cache write since allFromCache is undefined and offsets are hardcoded
#ifndef SERAPH_DMA_BUILD
        if (!allFromCache) {
            AobCacheRec out;
            (void)AobCache_Read(d2Base, d2Tds, &out);
            out.dmg_va      = feAll[0].result;
            out.hrg_va      = feAll[1].result;
            out.ik_va       = feAll[2].result;
            out.cam_aob_va  = feAll[3].result;
            out.hkp_aob_va  = feAll[4].result;
            out.rof_va      = feAll[5].result;
            out.ia_va       = feAll[6].result;
            out.ntb_va      = feAll[7].result;
            out.nja_va      = feAll[8].result;
            out.killaura_va = feAll[9].result;
            out.gs_va       = feAll[10].result;
            out.gsize_va    = feAll[11].result;
            out.ib_va       = feAll[12].result;
            out.itar_va     = feAll[13].result;
            out.ammo_va     = feAll[14].result;
            out.cloner_va   = feAll[15].result;
            out.brick_va    = feAll[16].result;
            out.hs_va       = feAll[17].result;
            out.movespeed_va= feAll[18].result;
            out.norecoil_va = feAll[19].result;
            out.silent_va   = feAll[20].result;
            out.features_valid = 1;
            AobCache_Write(d2Base, d2Tds, &out);
        }
#endif
        */
        /* ── Hardcoded offsets reference (for potential future use) ──
         *  Damage=0 Aura=0 (NOT FOUND).
         *  HealthRegen=+14506111  InstaKill=+23331098  Fly=+26397074
         *  Havok=+21423701  RapidFire=+14890268  InstAbils=+15256358
         *  NoTurnBack=+7459410  NoJoinAllies=+7131742  KillAura=+4803408
         *  GameSpeed=+35528736  Guardian=+53761644  ImmuneBoss=+13733388
         *  InteractAura=+7720365  InfiniteAmmo=+10611601
         *  PlayerCloner=+15355470  AmmoBrick=+13466831
         *  HSpeed=+14890276  MSpeed=+14772070                                */
    }

    /* Reset cave reservation table before any module scans for caves —
     * stale reservations from a previous attach cycle would block new scans. */
    WriteLogFile("FIT: CaveFinder_ClearReservations");
    CaveFinder_ClearReservations();

    /* Unconditionally initialize decryption keys first so all features (Fly, Teleports)
     * can perform decryption immediately upon attachment. */
    WriteLogFile("FIT: Keys_Init resolve");
    Keys_Init();
    
    /* Call ESP_Init to resolve the datum table VA (needed for bones coordinate translation in TigerList). */
    WriteLogFile("FIT: ESP_Init resolve");
    ESP_Init();

    #define _FIT_STEP(call) do{ WriteLogFile("FIT: " #call " START"); call; WriteLogFile("FIT: " #call " DONE"); }while(0)
    _FIT_STEP(Fly_OnAttach());
    _FIT_STEP(Matchmaking_Init());
#ifndef NDEBUG
    _FIT_STEP(NameChanger_OnAttach());  /* DEV-only */
#endif
    _FIT_STEP(GameSpeed_OnAttach());
    _FIT_STEP(Damage_OnAttach());
    _FIT_STEP(KillAura_OnAttach());
#if defined(SERAPH_DMA_BUILD) || !defined(NDEBUG)
    _FIT_STEP(Aura_OnAttach());
#endif
    _FIT_STEP(Guardian_OnAttach());
    _FIT_STEP(D2Patches_Register());
    _FIT_STEP(Revive_OnAttach());
    _FIT_STEP(InstantAbilities_OnAttach());
    _FIT_STEP(HealthRegen_OnAttach());
    _FIT_STEP(NoTurnBack_OnAttach());
    _FIT_STEP(NoJoinAllies_OnAttach());
    _FIT_STEP(NoInactivity_OnAttach());
    _FIT_STEP(InstaKill_OnAttach());
    _FIT_STEP(ImmuneBoss_OnAttach());
    _FIT_STEP(InteractAura_OnAttach());
    _FIT_STEP(InfiniteAmmo_OnAttach());
    _FIT_STEP(Ammo_OnAttach());
    _FIT_STEP(AmmoBrick_OnAttach());
    _FIT_STEP(HSpeed_OnAttach());
    _FIT_STEP(MSpeed_OnAttach());
    _FIT_STEP(LP_OnAttach());
    _FIT_STEP(PlayerCloner_OnAttach());
    _FIT_STEP(RapidFire_OnAttach());
    _FIT_STEP(WeaponStats_OnAttach());
    _FIT_STEP(SilentAim_OnAttach());
    _FIT_STEP(NoRecoil_OnAttach());
    _FIT_STEP(ThirdPerson_OnAttach());
    _FIT_STEP(SpinBot_OnAttach());

    #undef _FIT_STEP
    /* ── Final all-OK gate: after every OnAttach has had a chance to run its
     * own fallback scan, query each feature's IsReady().  This means a
     * feature that missed in the multi-scan but found its target via
     * fallback still counts as ready, and "cheat initialized" fires. */
    {
        BOOL allOk = TRUE;
        struct { BOOL (*ready)(void); const char* name; } checks[] = {
            { Damage_IsReady,      "Damage"      },
            { RapidFire_IsReady,   "RapidFire"   },
            { GameSpeed_IsReady,   "GameSpeed"   },
            { Guardian_IsReady,    "Guardian"    },
            { ImmuneBoss_IsReady,  "ImmuneBoss"  },
            /* InteractAura/InfiniteAmmo/PlayerCloner are optional extras —
             * their AOBs may not match every D2 version.  Exclude from the
             * mandatory gate so a missing extra does not block core attach. */
        };
        for (int i = 0; i < (int)(sizeof(checks)/sizeof(checks[0])); i++) {
            if (!checks[i].ready()) {
                char b[160]; wsprintfA(b,"FIT: feature NOT READY: '%s'", checks[i].name);
                WriteLogFile(b);
                allOk = FALSE;
            }
        }
        int patchFails = D2Patches_GetFailCount();
        if (patchFails > 0) {
            char b[96]; wsprintfA(b,"FIT: %d D2Patch AOB(s) failed", patchFails);
            WriteLogFile(b);
            allOk = FALSE;
        }
    ...
    for (;;) {
        ...
        while (GetDestiny2CR3() == 0) {
            ...
            while (GetDestiny2CR3() == 0 && retries < 10) {
                if (AttachToDestiny2()) {
                    /* Fly_OnAttach() -- disabled */
                    /* Ammo_OnAttach(); -- disabled */
        ...
        /* Phase 2 alive check: probe-read the PE MZ header from game base.
         * More robust than Destiny2ProcessFound (EPROCESS walk) which EAC
         * can interfere with post-attach.  If BYOVD can still read the first
         * 2 bytes of the game image and they are MZ (0x5A4D), process is live.
         * 12 consecutive failures (~6s) = game truly closed. */
        int missCount = 0;
        while (GetDestiny2CR3() != 0) {
            InterlockedExchange(&s_manualAttachPending, 0);
            SeraphSleep(500);
            UINT64 _cr3  = GetDestiny2CR3();
            UINT64 _base = GetDestiny2Base();
            UINT16 _mz   = 0;
            BOOL   _alive = FALSE;
            if (_cr3 && _base) {
                BYOVD_LOCK();
                _alive = BYOVD_ReadVA_NoCache(_cr3, _base, &_mz, sizeof(_mz)) && _mz == 0x5A4Du;
                BYOVD_UNLOCK();
                if (!_alive) {
                    _alive = Destiny2ProcessFound();
                }
            }
            if (!_alive) {
                missCount++;
                { char _mb[80]; wsprintfA(_mb, "AutoAttach: probe miss #%d/120", missCount); WriteLogFile(_mb); }
                if (missCount < 120) continue; /* ~60s antes de detach */
                WriteLogFile("AutoAttach: 120 probe misses — invalidate, saindo do cheat");
                while (s_featureInitPending) SeraphSleep(50);
                Attach_Invalidate();
                ...
                Overlay_Stop();
                if (g_hW) {
                    PostMessageA(g_hW, WM_CLOSE, 0, 0);
                }
                break;
            }
            missCount = 0;
...
static void RebuildNav() {
    g_tE[0].clear();
    int sW = 220;
    float startY = 195.f;
    float stepY = 40.f;
    
    // 1. PLAYER Category Tab
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"PLAYER"});
    startY += stepY;
    
    // 2. Subtabs (Movement, Abilities, Weapons, Health)
    float subOpacity = g_playerMenuOpacity;
    if (subOpacity > 0.001f) {
        int subH = 30;
        int indent = 28;
        float subStep = 36.f;
        
        const wchar_t* subTabs[] = { L"General", L"Movement", L"Abilities", L"Weapons", L"Health" };
        for (int i = 0; i < 5; i++) {
            float yPos = startY + i * subStep * subOpacity;
            g_tE[0].push_back({UE::TabButton, {indent, (int)yPos, sW - 12, (int)yPos + subH}, 0, 0, 0, 0, subTabs[i]});
        }
        startY += 5.f * subStep * subOpacity;
    }
    
    // 3. MISC
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"MISC"});
    startY += stepY;
    
    // 4. SETTINGS
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"SETTINGS"});
    startY += stepY;
    
    // 5. DEV
#ifndef NDEBUG
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"DEV"});
    startY += stepY;
#endif

    // 6. TELEPORTS / ESP / AIMBOT
#ifndef SERAPH_EXCLUDE_ESP
# ifndef NDEBUG
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"TELEPORTS"});
    startY += stepY;
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"ESP"});
    startY += stepY;
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"AIMBOT"});
    startY += stepY;
# else
    g_tE[0].push_back({UE::TabButton, {12, (int)startY, sW - 12, (int)startY + 32}, 0, 0, 0, 0, L"TELEPORTS"});
    startY += stepY;
...
void InitUI(){
    ...
    RebuildNav();
    g_aT=2;
    {HANDLE _hT=SeraphCreateThread(AutoAttachThread,NULL);if(_hT)SysNtClose(_hT);}}
...
if (g_showTpConfigModal) {
    if (clicked || clickedRight) {
        if (g_tpConfigModalMode == 1) {
            ...
            if (clicked && g_mX >= g_tpCloseR.left && g_mX <= g_tpCloseR.right && g_mY >= g_tpCloseR.top && g_mY <= g_tpCloseR.bottom) {
            ...
            if (clicked && (g_mX < g_tpModalR.left || g_mX > g_tpModalR.right || g_mY < g_tpModalR.top || g_mY > g_tpModalR.bottom)) {
                g_showTpConfigModal = false;
                Overlay_TextInputDefocus();
                Overlay_RebuildDevTab();
                return;
            ...
            /* Load modal: handle config list item clicks directly */
            {
                ...
                for (int i = 0; i < tpConfigsCount; i++) {
                    ...
                    if (g_mX >= bx && g_mX <= bx + bw && g_mY >= iy && g_mY <= iy + itemH) {
                        float x_left = bx + bw - 22.0f;
                        float x_right = bx + bw - 6.0f;
                        float x_top = iy + 6.0f;
                        float x_bottom = iy + itemH - 6.0f;
                        bool clickOnX = (g_mX >= x_left && g_mX <= x_right && g_mY >= x_top && g_mY <= x_bottom);
                        
                        char cfgName[64];
                        size_t converted = 0;
                        wcstombs_s(&converted, cfgName, sizeof(cfgName), localTpConfigs[i], _TRUNCATE);
                        
                        if (clickedRight || (clicked && clickOnX)) {
                            TP_DeleteConfigFile(cfgName);
                            Overlay_RebuildDevTab();
                            Overlay_AddNotification(L"TP Config", L"TP config deleted.");
                            return;
                        } else if (clicked) {
...
if(g_lD){
    if (e.r.left > 12 && g_playerMenuOpacity < 0.5f) continue;
    if(e.txt==L"PLAYER"){
        static DWORD lastClick = 0;
        DWORD now = GetTickCount();
        if (now - lastClick > 350) {
            lastClick = now;
            g_playerMenuExpanded = !g_playerMenuExpanded;
            if (g_playerMenuExpanded) g_aT = 4;
            Overlay_RebuildDevTab();
        }
    }
    else if(e.txt==L"General"){g_aT=4; Overlay_RebuildDevTab();}
    else if(e.txt==L"Movement"){g_aT=5; Overlay_RebuildDevTab();}
    else if(e.txt==L"Abilities"){g_aT=10; Overlay_RebuildDevTab();}
    else if(e.txt==L"Weapons"){g_aT=6; Overlay_RebuildDevTab();}
    else if(e.txt==L"Health"){g_aT=11; Overlay_RebuildDevTab();}
#ifndef SERAPH_EXCLUDE_ESP
    else if(e.txt==L"TELEPORTS"){g_aT=9; Overlay_RebuildDevTab();}
    else if(e.txt==L"ESP"){g_aT=7; Overlay_RebuildDevTab();}
    else if(e.txt==L"AIMBOT"){g_aT=8; Overlay_RebuildDevTab();}
#endif
    else if(e.txt==L"MISC"){g_aT=1; Overlay_RebuildDevTab();}
    else if(e.txt==L"SETTINGS"){g_aT=2; Overlay_RebuildDevTab();}
#ifndef NDEBUG
    else if(e.txt==L"DEV"){g_aT=3; Overlay_RebuildDevTab();}
#endif
...
if(e.t==UE::Toggle && g_lD){DWORD _now=GetTickCount();if(_now-g_spLastToggle>350){g_spLastToggle=_now;*e.s=!*e.s;
    ...
    if(e.s==&g_flyActive){
        ...
        _needRebuild=true;
    } else if(e.s==&g_thirdPersonActive){
        ThirdPerson_SetEnabled(*e.s?TRUE:FALSE);
        _needRebuild=true;
#ifndef SERAPH_EXCLUDE_ESP
        ...
        Aimbot_SetMemoryAim(g_aimbotMemoryAim ? TRUE : FALSE);
    } else if(e.s==&g_aimbotTriggerbot){
        Aimbot_SetTriggerbot(g_aimbotTriggerbot ? TRUE : FALSE);
    } else if(e.s==&g_spinbotActive){
        SpinBot_SetEnabled(*e.s ? TRUE : FALSE);
        ...
        Guardian_SetEnabled(g_guardianActive?TRUE:FALSE);
        InstantSize_SetEnabled(g_guardianActive?TRUE:FALSE);
        if(!g_guardianActive){g_guardianValue=100; InstantSize_SetIntVal(100);}
        _needRebuild=true;
        ...
        _needRebuild=true;
#endif
    } else if(e.s==&g_wstatReloadActive || e.s==&g_wstatHandlingActive || e.s==&g_wstatRangeActive || e.s==&g_wstatAmmoActive){
        _needRebuild=true;
    } else if(e.s==&g_classAbilityActive || e.s==&g_meleeAbilityActive || e.s==&g_grenadeAbilityActive || e.s==&g_superAbilityActive){
        _needRebuild=true;
    ...
    if (g_showLoadConfigWindow) {
        if (e.txt == L"<- Back") {
            ...
            float x_left = (float)e.r.right - 26.0f;
            float x_right = (float)e.r.right - 6.0f;
            float x_top = (float)e.r.top + 6.0f;
            float x_bottom = (float)e.r.bottom - 6.0f;
            bool clickOnX = (g_mX >= x_left && g_mX <= x_right && g_mY >= x_top && g_mY <= x_bottom);
            
            if (clickedRight || (clicked && clickOnX)) {
                DeleteConfigJson(cfgName);
                Overlay_RebuildDevTab();
                Overlay_AddNotification(L"Config", L"Configuration deleted.");
            } else if (clicked) {
    ...
    if(clickedRight){
        ...
        InstantSize_SetIntVal(g_guardianValue);
        Overlay_RebuildDevTab();
    ...
    float _lO=(g_aT==4||g_aT==5||g_aT==10)?55.f:(g_aT==8?70.f:45.f);
    ...
    *e.v=e.min+(int)(frac*(float)(e.max-e.min)+0.5f);
...
#endif
    else if(e.v==&g_guardianValue) { Guardian_SetValue(g_guardianValue); InstantSize_SetIntVal(g_guardianValue); }
    ...
    else if(e.v==&g_thirdPersonDistance) ThirdPerson_SetDistance(2.0f + 28.0f * ((float)(g_thirdPersonDistance - 1) / 99.0f));
...
static void ProcessAbilitySpeedup(UINT64 cr3, UINT64 slotVA, UINT32 offset, float multiplier, DWORD dtMs, BOOL doLog) {
    if (multiplier <= 1.0f) return;
    UINT32 ticket = 0;
    BYOVD_LOCK();
    BOOL ok = BYOVD_ReadVA(cr3, slotVA + offset, &ticket, 4);
    BYOVD_UNLOCK();

    if (doLog) {
        LogAbilityMsg("ProcessAbilitySpeedup: offset=0x%X, ticket=0x%08X, readOk=%d",
            offset, ticket, (int)ok);
    }

    if (!ok || ticket == 0 || ticket == 0xFFFFFFFF) return;

    UINT64 abilityComp = DecryptTicket(cr3, ticket);
    if (doLog) {
        LogAbilityMsg("  resolved abilityComp=0x%llX", (unsigned long long)abilityComp);
    }

    if (abilityComp < 0x10000ULL) return;

    float currentCooldown = 0.0f;
    float maxCooldown = 0.0f;
    BYOVD_LOCK();
    BOOL r1 = BYOVD_ReadVA(cr3, abilityComp + 0xE9C, &currentCooldown, 4);
    BOOL r2 = BYOVD_ReadVA(cr3, abilityComp + 0xEA8, &maxCooldown, 4);
    BYOVD_UNLOCK();

    if (doLog) {
        LogAbilityMsg("  cooldown: current=%.4f, max=%.4f, mult=%.1f (read_cd_ok=%d, read_max_ok=%d)",
            currentCooldown, maxCooldown, multiplier, (int)r1, (int)r2);
    }

    if (r1 && r2 && currentCooldown > 0.0f && maxCooldown > 0.0f) {
        float dtSec = (float)dtMs / 1000.0f;
        float extra = (multiplier - 1.0f) * dtSec;
        float nextCooldown = currentCooldown - extra;
        if (nextCooldown < 0.0f) nextCooldown = 0.0f;

        BYOVD_LOCK();
        BOOL wOk = BYOVD_WriteVA(cr3, abilityComp + 0xE9C, &nextCooldown, 4);
        BYOVD_UNLOCK();
        
        if (doLog) {
            LogAbilityMsg("  cooldown update: from %.4f to %.4f (writeOk=%d)", currentCooldown, nextCooldown, (int)wOk);
        }
    }
}

static void Abilities_Tick(void) {
    static DWORD lastTick = 0;
    static DWORD lastLog  = 0;   /* throttle: log a cada ~2s */
    DWORD now = GetTickCount();
    if (lastTick == 0) { lastTick = now; lastLog = now; return; }
    DWORD dtMs = now - lastTick;
    if (dtMs < 10) return;
    lastTick = now;

    BOOL doLog = (now - lastLog >= 2000);
    if (doLog) lastLog = now;

    UINT64 cr3 = GetDestiny2CR3();
    if (!cr3) {
        if (doLog) LogAbilityMsg("Abilities_Tick: CR3 not found!");
        return;
    }

    int lpIdx = LP_GetLocalPlayerIndex();
    if (lpIdx < 0 || lpIdx >= TL_MAX_SLOTS) {
        if (doLog) LogAbilityMsg("Abilities_Tick: local player index invalid (%d)", lpIdx);
        return;
    }

    UINT64 container = TigerList_GetContainer();
    if (!container) {
        if (doLog) LogAbilityMsg("Abilities_Tick: TigerList container not found!");
        return;
    }
    ...
    BYOVD_UNLOCK();
    if (!ok || !arrPtr || arrPtr < 0x10000ULL) {
        if (doLog) LogAbilityMsg("Abilities_Tick: arrPtr read failed (ok=%d, val=0x%llX)", (int)ok, (unsigned long long)arrPtr);
        return;
    }
    ...
    if (doLog) {
        LogAbilityMsg("--- Abilities_Tick: cr3=0x%llX lpIdx=%d arrPtr=0x%llX slotVA=0x%llX ---",
            (unsigned long long)cr3, lpIdx,
            (unsigned long long)arrPtr, (unsigned long long)slotVA);
        LogAbilityMsg("Settings: classActive=%d(x%d) meleeActive=%d(x%d) grenadeActive=%d(x%d) superActive=%d(x%d)",
            g_classAbilityActive,   g_classAbilityMult,
            g_meleeAbilityActive,   g_meleeAbilityMult,
            g_grenadeAbilityActive, g_grenadeAbilityMult,
            g_superAbilityActive,   g_superAbilityMult);

        // Optimized heuristic memory scan over the realistic ability region
        LogAbilityMsg("Starting optimized slot scan (0x4600-0x48A0) for ability tickets...");
        int matchesFound = 0;
        for (UINT32 offset = 0x4600; offset < 0x48A0; offset += 4) {
            UINT32 ticket = 0;
            BYOVD_LOCK();
            BOOL rTicket = BYOVD_ReadVA(cr3, slotVA + offset, &ticket, 4);
            BYOVD_UNLOCK();

            if (rTicket && ticket != 0 && ticket != 0xFFFFFFFF) {
                // Try decoding
                UINT64 ptr = DecryptTicket(cr3, ticket);
                if (ptr >= 0x10000ULL && ptr < 0x00007FFFFFFFFFFULL) {
                    float currentCooldown = -1.0f;
                    float maxCooldown = -1.0f;
                    BYOVD_LOCK();
                    BOOL r1 = BYOVD_ReadVA(cr3, ptr + 0xE9C, &currentCooldown, 4);
                    BOOL r2 = BYOVD_ReadVA(cr3, ptr + 0xEA8, &maxCooldown, 4);
                    BYOVD_UNLOCK();

                    if (r1 && r2 && maxCooldown >= 5.0f && maxCooldown <= 600.0f && currentCooldown >= 0.0f && currentCooldown <= maxCooldown + 2.0f) {
                        LogAbilityMsg("[SCAN_MATCH] Found active ability at offset 0x%04X! ticket=0x%08X comp=0x%llX cd=%.2f max=%.2f",
                            offset, ticket, (unsigned long long)ptr, currentCooldown, maxCooldown);
                        matchesFound++;
                    }
                }
            }
        }
        LogAbilityMsg("Scan finished. Matches found: %d", matchesFound);
    }

    if (g_classAbilityActive) {
        if (doLog) LogAbilityMsg("[CLASS +0x4710]");
        ProcessAbilitySpeedup(cr3, slotVA, 0x4710, (float)g_classAbilityMult, dtMs, doLog);
    }
    if (g_meleeAbilityActive) {
        if (doLog) LogAbilityMsg("[MELEE +0x4718]");
        ProcessAbilitySpeedup(cr3, slotVA, 0x4718, (float)g_meleeAbilityMult, dtMs, doLog);
    }
    if (g_grenadeAbilityActive) {
        if (doLog) LogAbilityMsg("[GRENADE +0x4720]");
        ProcessAbilitySpeedup(cr3, slotVA, 0x4720, (float)g_grenadeAbilityMult, dtMs, doLog);
    }
    if (g_superAbilityActive) {
        if (doLog) LogAbilityMsg("[SUPER +0x4728]");
        ProcessAbilitySpeedup(cr3, slotVA, 0x4728, (float)g_superAbilityMult, dtMs, doLog);
    }
}
...
// ShowWindow(SW_HIDE) in the gui.c hotkey handler.
    ...
    CM();
    if (g_playerMenuExpanded) {
        if (g_playerMenuOpacity < 1.0f) {
            g_playerMenuOpacity += 0.15f;
            if (g_playerMenuOpacity > 1.0f) g_playerMenuOpacity = 1.0f;
        }
    } else {
        if (g_playerMenuOpacity > 0.0f) {
            g_playerMenuOpacity -= 0.15f;
            if (g_playerMenuOpacity < 0.0f) g_playerMenuOpacity = 0.0f;
        }
    }
    RebuildNav();
...
for(auto& e:g_tE[0]){
    ...
    bool act=false;
    if (e.txt == L"PLAYER") act = (g_aT == 4 || g_aT == 5 || g_aT == 6 || g_aT == 10 || g_aT == 11);
    else if (e.txt == L"General") act = (g_aT == 4);
    else if (e.txt == L"Movement") act = (g_aT == 5);
    else if (e.txt == L"Abilities") act = (g_aT == 10);
    else if (e.txt == L"Weapons") act = (g_aT == 6);
    else if (e.txt == L"Health") act = (g_aT == 11);
    else if (e.txt == L"MISC") act = (g_aT == 1);
    else if (e.txt == L"SETTINGS") act = (g_aT == 2);
#ifndef NDEBUG
    else if (e.txt == L"DEV") act = (g_aT == 3);
#endif
#ifndef SERAPH_EXCLUDE_ESP
    else if (e.txt == L"TELEPORTS") act = (g_aT == 9);
    else if (e.txt == L"ESP") act = (g_aT == 7);
    else if (e.txt == L"AIMBOT") act = (g_aT == 8);
#endif

    bool isSubtab = (e.r.left > 12);
    float finalAlpha = 1.0f;
    if (isSubtab) {
        finalAlpha = g_playerMenuOpacity;
        if (finalAlpha <= 0.01f) continue;
    }

    if(act){
        DR(e.r.left,e.r.top,e.r.right,e.r.bottom,0.0f,0.898f,1.0f,0.10f * finalAlpha);
        
        float barR = 0.0f, barG = 0.898f, barB = 1.0f, barA = 0.9f;
        {auto*b=Br(barR,barG,barB,barA*finalAlpha);if(b){g_rT->DrawLine({(float)e.r.left,(float)e.r.top},{(float)e.r.left,(float)e.r.bottom},b,3.f);b->Release();}}
    }
    else if(hv) {
        DR(e.r.left,e.r.top,e.r.right,e.r.bottom,0.0f,0.898f,1.0f,0.03f*finalAlpha);
    } else if(isSubtab) {
        {auto*b=Br(0.3f,0.3f,0.3f,0.5f*finalAlpha);if(b){g_rT->DrawLine({(float)e.r.left,(float)e.r.top},{(float)e.r.left,(float)e.r.bottom},b,2.f);b->Release();}}
    }

    float ty=e.r.top+(e.r.bottom-e.r.top)*0.5f-9.f;
    float txtR = 0.6f, txtG = 0.6f, txtB = 0.6f;
    if (act) {
        txtR = 0.0f; txtG = 0.898f; txtB = 1.0f;
    } else if (hv) {
        txtR = 0.9f; txtG = 0.9f; txtB = 0.9f;
    }
    auto*b=Br(txtR,txtG,txtB,1.0f*finalAlpha);
    if(b){
        D2D1_RECT_F r={(float)e.r.left+15,ty,(float)e.r.right,ty+24};
        g_mTF->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_LEADING);
        if (e.txt == L"PLAYER") {
            const wchar_t* dispText = L"\x26A1  Player";
            g_rT->DrawText(dispText, 9, g_mTF, r, b);
        } else {
            g_rT->DrawText(e.txt.c_str(),(UINT32)e.txt.size(),g_mTF,r,b);
        }
        b->Release();
    }}
// ===CONTENT===
...
if(g_aT==4)title=L"GENERAL";
else if(g_aT==10)title=L"ABILITIES";
else if(g_aT==5)title=L"MOVEMENT";
else if(g_aT==6)title=L"WEAPONS";
else if(g_aT==11)title=L"HEALTH";
#ifndef SERAPH_EXCLUDE_ESP
...
auto*kb=Br(0.533f,0.533f,0.533f,1.0f);if(kb){g_rT->FillEllipse(D2D1::Ellipse({tx2+3+8,ty2+th/2},8,8),kb);kb->Release();}
    ...
    if(g_aT==10 && e.v==&g_classAbilityMult && !g_classAbilityActive) continue;
    if(g_aT==10 && e.v==&g_meleeAbilityMult && !g_meleeAbilityActive) continue;
    if(g_aT==10 && e.v==&g_grenadeAbilityMult && !g_grenadeAbilityActive) continue;
    if(g_aT==10 && e.v==&g_superAbilityMult && !g_superAbilityActive) continue;
    float lOff = (e.txt.size() > 6) ? 70.f : ((e.txt.size() > 4) ? 55.f : 45.f), rOff = 28.f;
...
if(e.txt==L"Change Key"||e.txt==L"Fly Hotkey"||e.txt==L"Fly Dir Hotkey"||e.txt==L"Gs Hotkey"||e.txt==L"Opk Hotkey"||e.txt==L"Aimbot Hotkey"||e.txt==L"Target Head Hotkey"||e.txt==L"Suicide Hotkey"||e.txt==L"Fuser Hotkey"){
    ...
    if(lbT){D2D1_RECT_F rlT={(float)e.r.left+20,ltyT,(float)e.r.right-110,ltyT+24};g_mTF->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_LEADING);g_rT->DrawText(L"Target",6,g_mTF,rlT,lbT);lbT->Release();}
    const wchar_t* valS=g_aimbotHeadTarget?L"Head":L"Body";
    D2D1_RECT_F pillR={(float)e.r.right-76,(float)e.r.top+6,(float)e.r.right-20,(float)e.r.bottom-6};
    ...
    if (g_aT == 9 && wcscmp(e.txt.c_str(), L"Save TP") == 0) {
        ...
        bool isSettingConfigItem = g_showLoadConfigWindow && (g_aT == 2) && (e.txt != L"<- Back");
        auto rrB=D2D1::RoundedRect(D2D1::RectF((float)e.r.left,(float)e.r.top,(float)e.r.right,(float)e.r.bottom),4,4);
        ...
        if (isSettingConfigItem) {
            float x_left = (float)e.r.right - 26.0f;
            float x_right = (float)e.r.right - 6.0f;
            float x_top = (float)e.r.top + 6.0f;
            float x_bottom = (float)e.r.bottom - 6.0f;
            bool hoverX = (g_mX >= x_left && g_mX <= x_right && g_mY >= x_top && g_mY <= x_bottom);
            
            // Draw text slightly offset to the left
            DT_C(e.txt.c_str(),(float)e.r.left + 10.0f,(float)e.r.top,(float)e.r.right - 30.0f,(float)e.r.bottom,0.0f,0.898f,1.0f,1.0f,g_sTF);
            // Draw small 'x'
            DT_C(L"x", x_left, x_top, x_right, x_bottom, hoverX ? 1.0f : 0.0f, hoverX ? 0.2f : 0.898f, hoverX ? 0.2f : 1.0f, hoverX ? 1.0f : 0.4f, g_sTF);
        } else {
    ...
    if (g_tpConfigModalMode == 1) {
        ...
        if (tpConfigsCount == 0) {
            ...
            for (int i = 0; i < tpConfigsCount; i++) {
                ...
                // Draw text slightly offset to the left
                DT_C(cfgW, bx + 10, iy, bx + bw - 26, iy + itemH, 0.0f, 0.898f, 1.0f, 1.0f * fade, g_mTF);
                // Draw small 'x'
                DT_C(L"x", x_left, x_top, x_right, x_bottom, hoverX ? 1.0f : 0.0f, hoverX ? 0.2f : 0.898f, hoverX ? 0.2f : 1.0f, (hoverX ? 1.0f : 0.4f) * fade, g_sTF);
                shown++;
            }
        }
    } else if (g_tpConfigModalMode == 2) {
...
void Overlay_StartFeatureInit(void){
    /* Idempotent: s_attachThreadActive guard inside AutoAttachThread prevents duplicate threads. */
    {HANDLE _hT=SeraphCreateThread(AutoAttachThread,NULL);if(_hT)SysNtClose(_hT);}
}
...
void Overlay_RebuildDevTab_Impl(){
    ...
    g_tE[10].clear();  /* ABILITIES tab */
    g_tE[11].clear();  /* HEALTH tab */
    /* === SETTING tab content === */
    if (g_showLoadConfigWindow) {
        int pyS = hH + 70;
    ...
    int pyMisc=hH+70, pyDev=hH+70, pyPlayer=hH+70, pyMovement=hH+70, pyWeapons=hH+70, pyEsp=hH+70, pyAimbot=hH+70, pyTeleports=hH+70, pyAbilities=hH+70, pyHealth=hH+70, pyGeneral=hH+70;
#ifndef SERAPH_EXCLUDE_ESP
    ...
    pyAimbot+=40;
    g_tE[8].push_back({UE::Toggle,{cX,pyAimbot,cE,pyAimbot+32},&g_spinbotActive,0,0,0,L"Spinbot"});
    pyAimbot+=40;
    g_tE[8].push_back({UE::Toggle,{cX,pyAimbot,cE,pyAimbot+32},&g_aimbotTriggerbot,0,0,0,L"Triggerbot"});
    pyAimbot+=40;
#ifdef SERAPH_DMA_BUILD
    ...
    if(aimAny){
        ...
        if (g_waitingForAimbotTargetHeadKey) {
            ...
            g_tE[8].push_back({UE::TabButton,{cE-100,pyAimbot+8,cE-84,pyAimbot+24},0,0,0,0,L"TargetHeadGear"});
...
#endif
    /* === TELEPORTS tab (always available) === */
    /* TP name input */
    g_tE[9].push_back({UE::TextInput,{cX,pyTeleports,cE-100,pyTeleports+32},0,0,0,0,L"tp_name"});
    /* Save TP button */
    g_tE[9].push_back({UE::TabButton,{cE-90,pyTeleports,cE,pyTeleports+32},0,0,0,0,L"Save TP"});
    pyTeleports+=42;
    pyTeleports+=15;
    /* TP list */
    {
        int tpCount = TP_GetCount();
        for (int ti = 0; ti < tpCount && ti < TP_MAX_SLOTS; ti++) {
            const TPSlot* slot = TP_GetSlot(ti);
            if (!slot || !slot->valid) continue;
            if (g_waitingForTpKey && g_tpWaitingSlot == ti) {
                wchar_t tpDisplay[TP_NAME_MAX + 40];
                wsprintfW(tpDisplay, L"(%d) %s   Press any key...", ti, slot->name);
                g_tE[9].push_back({UE::TabButton,{cX,pyTeleports,cE,pyTeleports+40},0,0,0,0,tpDisplay});
            } else {
                wchar_t tpDisplay[TP_NAME_MAX + 64];
                wsprintfW(tpDisplay, L"(%d) %s   [%d, %d, %d]", ti, slot->name, (int)slot->x, (int)slot->y, (int)slot->z);
                g_tE[9].push_back({UE::TabButton,{cX,pyTeleports,cE-30,pyTeleports+40},0,0,0,0,tpDisplay});
                wchar_t gearLabel[32];
                wsprintfW(gearLabel, L"TpGear_%d", ti);
                g_tE[9].push_back({UE::TabButton,{cE-30,pyTeleports+8,cE-14,pyTeleports+32},0,0,0,0,gearLabel});
            }
            pyTeleports+=40;
        }
    }
    /* ── TP Save/Load buttons at bottom-right (same style as settings) ── */
    g_tE[9].push_back({UE::TabButton,{W-240,H-fH-45,W-130,H-fH-17},0,0,0,0,L"Load Config"});
    g_tE[9].push_back({UE::TabButton,{W-120,H-fH-45,W-10,H-fH-17},0,0,0,0,L"Save Configs"});
    /* === Fly WASD: row morphs between Toggle (with gear) and amber rebind row.
     * While waiting for a key, gear hides and the row is the rebind prompt.
     * After SetFlyHotkey clears the waiting flag, rebuild restores toggle+gear. */
    if(g_waitingForFlyKey){
        g_tE[5].push_back({UE::TabButton,{cX,pyMovement,cE,pyMovement+32},0,0,0,0,L"Fly Hotkey"});
    } else {
        g_tE[5].push_back({UE::TabButton,{cE-100,pyMovement+8,cE-84,pyMovement+24},0,0,0,0,L"FlyGear"});
        g_tE[5].push_back({UE::Toggle,{cX,pyMovement,cE,pyMovement+32},&g_flyActive,0,0,0,L"Fly WASD"});
    }
    pyMovement+=40;
    if(g_flyActive && !g_waitingForFlyKey){
        g_tE[5].push_back({UE::Slider,{cX,pyMovement,cE,pyMovement+32},0,&g_flySpeed,1,10,L"Speed"});
        pyMovement+=40;
    }
    /* === Fly Directional: same morph behavior === */
    if(g_waitingForFlyDirKey){
        g_tE[5].push_back({UE::TabButton,{cX,pyMovement,cE,pyMovement+32},0,0,0,0,L"Fly Dir Hotkey"});
    } else {
        g_tE[5].push_back({UE::TabButton,{cE-100,pyMovement+8,cE-84,pyMovement+24},0,0,0,0,L"FlyDirGear"});
        g_tE[5].push_back({UE::Toggle,{cX,pyMovement,cE,pyMovement+32},&g_flyDirActive,0,0,0,L"Fly Directional"});
    }
    pyMovement+=40;
    if(g_flyDirActive && !g_waitingForFlyDirKey){
        g_tE[5].push_back({UE::Slider,{cX,pyMovement,cE,pyMovement+32},0,&g_flyDirSpeed,1,10,L"Speed"});
        pyMovement+=40;
    }
    /* === Movement Speed === */
    g_tE[5].push_back({UE::Toggle,{cX,pyMovement,cE,pyMovement+32},&g_msActive,0,0,0,L"Movement Speed"});
    pyMovement+=40;
    if(g_msActive){
        g_tE[5].push_back({UE::Slider,{cX,pyMovement,cE,pyMovement+32},0,&g_msValue,1,10,L"Speed"});
        pyMovement+=40;
    }
    /* === Third Person === */
    g_tE[5].push_back({UE::Toggle,{cX,pyMovement,cE,pyMovement+32},&g_thirdPersonActive,0,0,0,L"Third Person"});
    pyMovement+=40;
    if (g_thirdPersonActive) {
        g_tE[5].push_back({UE::Slider,{cX,pyMovement,cE,pyMovement+32},0,&g_thirdPersonDistance,1,100,L"Distance"});
        pyMovement+=40;
    }
    /* === Game Speed: toggle (with gear) morphs into amber rebind row when waiting === */
    if(g_waitingForGsKey){
        g_tE[1].push_back({UE::TabButton,{cX,pyMisc,cE,pyMisc+32},0,0,0,0,L"Gs Hotkey"});
    } else {
        g_tE[1].push_back({UE::TabButton,{cE-100,pyMisc+8,cE-84,pyMisc+24},0,0,0,0,L"GsGear"});
        g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_gsActive,0,0,0,L"Game Speed"});
    }
    pyMisc+=40;
    if(g_gsActive && !g_waitingForGsKey){
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE,pyMisc+32},0,&g_gsSpeed,1,100,L"Speed"});
        pyMisc+=40;
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE,pyMisc+32},0,&g_gsSlow,1,10,L"Slow"});
        pyMisc+=40;
    }
    /* === Damage Increase toggle + conditional slider + number input === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_dmgActive,0,0,0,L"Damage Increase"});
    pyMisc+=40;
    if(g_dmgActive){
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE-85,pyMisc+32},0,&g_dmgValue,1,1000,L"Dmg"});
        g_tE[1].push_back({UE::NumberInput,{cE-80,pyMisc,cE,pyMisc+32},0,&g_dmgValue,1,1000,L""});
        pyMisc+=40;
    }
    /* === Immune Bosses toggle === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_immuneBossActive,0,0,0,L"Immune Bosses"});
    pyMisc+=40;
    /* === Instakill toggle === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_instakillActive,0,0,0,L"Instakill"});
    pyMisc+=40;
    /* === Kill Aura toggle + conditional slider (settings tab) === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_killAuraActive,0,0,0,L"Kill Aura"});
    pyMisc+=40;
    if(g_killAuraActive){
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE,pyMisc+32},0,&g_killAuraValue,1,100,L"Size"});
        pyMisc+=40;
    }
    /* === Interact Aura toggle === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_itarActive,0,0,0,L"Interact Aura"});
    pyMisc+=40;
    /* === Ammo Brick Spawner toggle === */
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_ammoBrickActive,0,0,0,L"Ammo Brick Spawner"});
    pyMisc+=40;
#if defined(SERAPH_DMA_BUILD) || !defined(NDEBUG)
    /* === Immune Aura toggle + conditional slider === */
#if defined(SERAPH_DMA_BUILD)
    g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_auraActive,0,0,0,L"Immune Aura"});
    pyMisc+=40;
    if(g_auraActive){
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE,pyMisc+32},0,&g_auraValue,1,100,L"Size"});
        pyMisc+=40;
    }
#else
    g_tE[3].push_back({UE::Toggle,{cX,pyDev,cE,pyDev+32},&g_auraActive,0,0,0,L"Immune Aura"});
    pyDev+=40;
    if(g_auraActive){
        g_tE[3].push_back({UE::Slider,{cX,pyDev,cE,pyDev+32},0,&g_auraValue,1,100,L"Size"});
        pyDev+=40;
    }
#endif
#endif
    /* === OPK (One Position Kill) toggle + conditional slider === */
    if(g_waitingForOpkKey){
        g_tE[1].push_back({UE::TabButton,{cX,pyMisc,cE,pyMisc+32},0,0,0,0,L"Opk Hotkey"});
    } else {
        g_tE[1].push_back({UE::TabButton,{cE-100,pyMisc+8,cE-84,pyMisc+24},0,0,0,0,L"OpkGear"});
        g_tE[1].push_back({UE::Toggle,{cX,pyMisc,cE,pyMisc+32},&g_opkActive,0,0,0,L"One Position Kill"});
    }
    pyMisc+=40;
    if(g_opkActive && !g_waitingForOpkKey){
        g_tE[1].push_back({UE::Slider,{cX,pyMisc,cE,pyMisc+32},0,&g_opkDistance,1,5,L"Distance"});
        pyMisc+=40;
    }
    /* === Rapid Fire (WEAPONS tab) + conditional multiplier slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_rofActive,0,0,0,L"Rapid Fire"});
    pyWeapons+=40;
    if(g_rofActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_rofMult,1,10,L"Mult"});
        pyWeapons+=40;
    }
#ifndef SERAPH_DMA_BUILD
    /* === Reload Boost (WEAPONS tab) + conditional multiplier slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_wstatReloadActive,0,0,0,L"Reload Boost"});
    pyWeapons+=40;
    if(g_wstatReloadActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_wstatReloadMult,1,10,L"Mult"});
        pyWeapons+=40;
    }
    /* === Handling Boost (WEAPONS tab) + conditional multiplier slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_wstatHandlingActive,0,0,0,L"Handling Boost"});
    pyWeapons+=40;
    if(g_wstatHandlingActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_wstatHandlingMult,1,10,L"Mult"});
        pyWeapons+=40;
    }
    /* === Range Boost (WEAPONS tab) + conditional multiplier slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_wstatRangeActive,0,0,0,L"Range Boost"});
    pyWeapons+=40;
    if(g_wstatRangeActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_wstatRangeMult,1,10,L"Mult"});
        pyWeapons+=40;
    }
    /* === Infinite Ammo (WEAPONS tab) === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_ammoActive,0,0,0,L"Infinite Ammo"});
    pyWeapons+=40;
    /* === Set Ammo (WEAPONS tab) + conditional value slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_wstatAmmoActive,0,0,0,L"Set Ammo"});
    pyWeapons+=40;
    if(g_wstatAmmoActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_wstatAmmoValue,1,99,L"Amount"});
        pyWeapons+=40;
    }
#endif
    /* === Silent Aim (WEAPONS tab) + conditional cone slider === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_silentActive,0,0,0,L"Silent Aim"});
    pyWeapons+=40;
    if(g_silentActive){
        g_tE[6].push_back({UE::Slider,{cX,pyWeapons,cE,pyWeapons+32},0,&g_silentValue,1,100,L"Cone"});
        pyWeapons+=40;
    }
    /* === No Recoil (WEAPONS tab) === */
    g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_noRecoilActive,0,0,0,L"No Recoil"});
    pyWeapons+=40;
    /* === Suicide (instant TP to Y=-700) — clickable button with gear hotkey === */
    if(g_waitingForSuicideKey){
        g_tE[4].push_back({UE::TabButton,{cX,pyGeneral,cE,pyGeneral+32},0,0,0,0,L"Suicide Hotkey"});
    } else {
        g_tE[4].push_back({UE::TabButton,{cE-52,pyGeneral+5,cE-20,pyGeneral+27},0,0,0,0,L"SuicideGear"});
        g_tE[4].push_back({UE::TabButton,{cX,pyGeneral,cE,pyGeneral+32},0,0,0,0,L"Suicide"});
    }
    pyGeneral+=40;
    /* === Guardian Size toggle + conditional slider + reset button === */
    g_tE[4].push_back({UE::Toggle,{cX,pyGeneral,cE,pyGeneral+32},&g_guardianActive,0,0,0,L"Guardian Size"});
    pyGeneral+=40;
    if(g_guardianActive){
        g_tE[4].push_back({UE::Slider,{cX,pyGeneral,cE,pyGeneral+32},0,&g_guardianValue,-1,2,L"Size"});
        pyGeneral+=40;
        g_tE[4].push_back({UE::TabButton,{cE-130,pyGeneral,cE,pyGeneral+26},0,0,0,0,L"Size Reset"});
        pyGeneral+=36;
    }
    /* === Player Cloner toggle === */
    g_tE[4].push_back({UE::Toggle,{cX,pyGeneral,cE,pyGeneral+32},&g_clonerActive,0,0,0,L"Player Cloner"});
    pyGeneral+=40;
    /* === Instant Respawn (Revive) toggle === */
    g_tE[4].push_back({UE::Toggle,{cX,pyGeneral,cE,pyGeneral+32},&g_reviveActive,0,0,0,L"Instant Respawn"});
    pyGeneral+=40;
#ifndef NDEBUG
    /* === Aimbot Vis Check (DEV tab — NaN gate, pending validation) === */
    g_tE[3].push_back({UE::Toggle,{cX,pyDev,cE,pyDev+32},&g_aimbotVisCheckActive,0,0,0,L"Vis Check"});
    pyDev+=40;
    /* === Name Changer (DEV tab) === */
    g_tE[3].push_back({UE::Toggle,{cX,pyDev,cE,pyDev+32},&g_ncActive,0,0,0,L"Name Changer"});
    pyDev+=40;
    if(g_ncActive){
        g_tE[3].push_back({UE::TextInput,{cX,pyDev,cE-85,pyDev+32},0,0,0,0,L""});
        g_tE[3].push_back({UE::TabButton,{cE-80,pyDev,cE,pyDev+32},0,0,0,0,L"Apply"});
        pyDev+=40;
    }
    /* === Activity Loader (DEV tab) === */
    g_tE[3].push_back({UE::Toggle,{cX,pyDev,cE,pyDev+32},&g_activityLoaderActive,0,0,0,L"Activity Loader"});
    pyDev+=40;
    if(g_activityLoaderActive){
        g_tE[3].push_back({UE::Slider,{cX,pyDev,cE-85,pyDev+32},0,&g_activityIdValue,0,9999,L"ID"});
        g_tE[3].push_back({UE::NumberInput,{cE-80,pyDev,cE,pyDev+32},0,&g_activityIdValue,0,9999,L""});
        pyDev+=40;
        g_tE[3].push_back({UE::TabButton,{cX,pyDev,cX+140,pyDev+32},0,0,0,0,L"Apply Activity"});
        pyDev+=40;
    }
#endif
    for(int pi=0;pi<Patch_Count();pi++){
        const char* pn=Patch_GetName(pi); if(!pn) continue;
        /* Exclude these features completely from the menu */
        //if(strcmp(pn,"No Turn Back")==0)    continue;
        //if(strcmp(pn,"No Joining Allies")==0) continue;
        if(strcmp(pn,"Infinite Sword Ammo")==0) continue;
        wchar_t wlabel[48]={0};
        for(int j=0;j<47&&pn[j];j++) wlabel[j]=(wchar_t)pn[j];
        int tab=D2Patches_GetTabForId(pi);
        if(tab==0) continue; /* Hidden internal patch */
        g_patchActive[pi]=Patch_IsApplied(pi)?true:false;
        if(tab==1){
            bool isStacks=(strcmp(pn,"Infinite Stacks")==0);
            int tRight=isStacks?cE-85:cE;
            g_tE[1].push_back({UE::Toggle,{cX,pyMisc,tRight,pyMisc+32},&g_patchActive[pi],0,0,0,wlabel});
            if(isStacks)
                g_tE[1].push_back({UE::NumberInput,{cE-80,pyMisc,cE,pyMisc+32},0,&g_stackCount,1,100,L""});
            pyMisc+=40;
        } else if(tab==11){
            g_tE[11].push_back({UE::Toggle,{cX,pyHealth,cE,pyHealth+32},&g_patchActive[pi],0,0,0,wlabel});
            pyHealth+=40;
        } else if(tab==10){
            g_tE[10].push_back({UE::Toggle,{cX,pyAbilities,cE,pyAbilities+32},&g_patchActive[pi],0,0,0,wlabel});
            pyAbilities+=40;
        } else if(tab==5){
            g_tE[5].push_back({UE::Toggle,{cX,pyMovement,cE,pyMovement+32},&g_patchActive[pi],0,0,0,wlabel});
            pyMovement+=40;
        } else if(tab==6){
            g_tE[6].push_back({UE::Toggle,{cX,pyWeapons,cE,pyWeapons+32},&g_patchActive[pi],0,0,0,wlabel});
            pyWeapons+=40;
        } else if(tab==4){
            g_tE[4].push_back({UE::Toggle,{cX,pyGeneral,cE,pyGeneral+32},&g_patchActive[pi],0,0,0,wlabel});
            pyGeneral+=40;
        } else {
            g_tE[3].push_back({UE::Toggle,{cX,pyDev,cE,pyDev+32},&g_patchActive[pi],0,0,0,wlabel});
            pyDev+=40;
        }
    }
    /* === ABILITIES tab custom speedup controls === */
    g_tE[10].push_back({UE::Toggle,{cX,pyAbilities,cE,pyAbilities+32},&g_classAbilityActive,0,0,0,L"Class Ability Speedup"});
    pyAbilities+=40;
    if(g_classAbilityActive){
        g_tE[10].push_back({UE::Slider,{cX,pyAbilities,cE,pyAbilities+32},0,&g_classAbilityMult,1,20,L"Mult"});
        pyAbilities+=40;
    }
    g_tE[10].push_back({UE::Toggle,{cX,pyAbilities,cE,pyAbilities+32},&g_meleeAbilityActive,0,0,0,L"Melee Ability Speedup"});
    pyAbilities+=40;
    if(g_meleeAbilityActive){
        g_tE[10].push_back({UE::Slider,{cX,pyAbilities,cE,pyAbilities+32},0,&g_meleeAbilityMult,1,20,L"Mult"});
        pyAbilities+=40;
    }
    g_tE[10].push_back({UE::Toggle,{cX,pyAbilities,cE,pyAbilities+32},&g_grenadeAbilityActive,0,0,0,L"Grenade Ability Speedup"});
    pyAbilities+=40;
    if(g_grenadeAbilityActive){
        g_tE[10].push_back({UE::Slider,{cX,pyAbilities,cE,pyAbilities+32},0,&g_grenadeAbilityMult,1,20,L"Mult"});
        pyAbilities+=40;
    }
    g_tE[10].push_back({UE::Toggle,{cX,pyAbilities,cE,pyAbilities+32},&g_superAbilityActive,0,0,0,L"Super Ability Speedup"});
    pyAbilities+=40;
    if(g_superAbilityActive){
        g_tE[10].push_back({UE::Slider,{cX,pyAbilities,cE,pyAbilities+32},0,&g_superAbilityMult,1,20,L"Mult"});
        pyAbilities+=40;
    }
}
